...
common_env() {
  printf 'GOOGLE_CLOUD_PROJECT=%s,USE_SECRET_MANAGER=%s,GCS_DATA_BUCKET=%s,TZ=%s,GCP_SECRET_PREFIX=%s,IKR_USE_CLOUD_SCHEDULER=1,GCP_REGION=%s,SCHEDULER_SA=%s' \
    "$GCP_PROJECT_ID" "$USE_SECRET_MANAGER" "$GCS_DATA_BUCKET" "$TZ" "$GCP_SECRET_PREFIX" "$GCP_REGION" "$SCHEDULER_SA"
}
