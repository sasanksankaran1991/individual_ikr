...
def render_users_content() -> None:
    """User management cards — used inside the unified Account tab (admin)."""
    ...
    if users:
        ...
        if reset_candidates:
            with card_container():
                ...
                st.caption("Set a new password without the old one.")
                ...
                confirm_pw = st.text_input(
                    "Confirm password", type="password", key="admin_reset_pw2"
                )
    ...
    if deletable:
        with card_container():
            ...
            st.caption("Permanently removes the user and all of their IKR data.")
...
def render_users_tab() -> None:
    """Standalone users page (legacy entry point)."""
